...
ported_fns! {
    ...
    pub(crate) fn create_named_pipe_client(name: &std::ffi::OsStr, is_async: bool) -> RawFd {
        ...
        use windows_sys::Win32::Storage::FileSystem::{
            CreateFileW, FILE_FLAG_OVERLAPPED, GENERIC_READ, OPEN_EXISTING,
        };
    ...
    pub(crate) fn get_atime_sec(file_time: &FileTime) -> i64 {
        #[cfg(windows)]
        {
            file_time.LastAccessTime.QuadPart() / 10_000_000
        }
        #[cfg(unix)]
        {
    ...
    pub(crate) fn get_mtime_sec(file_time: &FileTime) -> i64 {
        #[cfg(windows)]
        {
            file_time.LastWriteTime.QuadPart() / 10_000_000
        }
        #[cfg(unix)]
        {
    ...
    pub(crate) fn get_ctime_sec(file_time: &FileTime) -> i64 {
        #[cfg(windows)]
        {
            file_time.ChangeTime.QuadPart() / 10_000_000
        }
        #[cfg(unix)]
        {
